            ...
            <div id="view-return-process" style="<?php echo $is_auto ? 'display: block;' : 'display: none;'; ?>">
                ...
                <!-- SECTION 1: Detail Pesanan -->
                                ...
                                <tbody id="rincian-item-list">
                                    <?php if ($is_auto): ?>
                                        <tr style="border-bottom: 1px solid #f1f5f9;">
                                            <td class="fw-bold py-3 ps-0 text-secondary" width="15%"><?= $lang['lbl_info_order'] ?? 'Info Order' ?></td>
                                            <td class="py-3 text-dark">: <?php echo !empty($auto_detail) ? htmlspecialchars($auto_detail) : ($lang['lbl_detail_req_trx'] ?? 'Detail Request Transaksi #') . htmlspecialchars($auto_id_transaksi); ?></td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                    ...
                    <div class="bg-white border rounded-3 p-4 mb-4 shadow-sm">
                        <h6 class="fw-bold mb-4" style="color: #4b5563;">
                            <i class="bi bi-arrow-counterclockwise me-2"></i><?= $lang['ret_sect_tiket'] ?? 'Informasi Tiket Return' ?>
                        </h6>
                        <div class="row g-4">
                            <div class="col-md-6 col-lg-3">
                                <label class="form-label fw-semibold text-secondary" style="font-size: 13px;"><?= $lang['lbl_id_transaksi'] ?? 'ID Transaksi' ?></label>
                                <input type="text" class="form-control <?php echo $bg_class; ?>" id="input_no_return" name="no_return" value="<?php echo htmlspecialchars($auto_id_transaksi); ?>" placeholder="<?= $lang['plc_id_transaksi'] ?? 'Contoh: TRX-000001' ?>" readonly>
                            </div>
                            <div class="col-md-6 col-lg-3">
                                <label class="form-label fw-semibold text-secondary" style="font-size: 13px;"><?= $lang['trx_lbl_id_sales'] ?? 'ID Sales' ?></label>
                                <input type="text" class="form-control <?php echo $bg_class; ?>" id="input_id_sales" name="id_request_awal" value="<?php echo htmlspecialchars($auto_sales_id); ?>" placeholder="<?= $lang['trx_lbl_id_sales'] ?? 'ID Sales' ?>" readonly>
                            </div>
                            <div class="col-md-6 col-lg-3">
                                <label class="form-label fw-semibold text-secondary" style="font-size: 13px;"><?= $lang['trx_lbl_nama_sa'] ?? 'Nama SA' ?></label>
                                <input type="text" class="form-control <?php echo $bg_class; ?>" id="input_nama" name="nama" value="<?php echo htmlspecialchars($auto_nama); ?>" placeholder="<?= $lang['trx_lbl_nama_sa'] ?? 'Nama SA' ?>" readonly>
                            </div>
                            <div class="col-md-6 col-lg-3">
                                <label class="form-label fw-semibold text-secondary" style="font-size: 13px;"><?= $lang['lbl_tgl_return'] ?? 'Tanggal Return' ?></label>
                    ...
                    <div class="bg-white border rounded-3 p-4 mb-4 shadow-sm">
                        <h6 class="fw-bold mb-3" style="color: #b91c1c;">
                            <i class="bi bi-box-seam me-2"></i><?= $lang['ret_list_item'] ?? 'Daftar Item Barang dalam Transaksi Ini' ?>
                        </h6>
                        <p class="text-muted small mb-3"><?= $lang['ret_desc_kondisi'] ?? 'Tentukan kondisi barang untuk setiap item yang dikembalikan di bawah ini:' ?></p>
